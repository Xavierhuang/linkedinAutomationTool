  ...
  const handleGenerateNow = async (campaignId) => {
    try {
      const response = await axios.post(`${BACKEND_URL}/api/ai-content/generate`, {
        ...
        org_id: orgId,
        user_id: user.id  // Let backend retrieve API keys from settings based on campaign's text_model
      });
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 md:px-8 py-4 md:py-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gray-900">AI-Powered Campaigns</h1>
            <p className="text-xs md:text-sm text-gray-600 mt-1">Autonomous LinkedIn content generation and posting</p>
          </div>
          <Button
            onClick={handleCreateCampaign}
            className="bg-gray-900 hover:bg-gray-800 text-white flex items-center justify-center gap-2 h-10 md:h-auto px-4 text-sm md:text-base whitespace-nowrap"
          >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6 max-w-7xl mx-auto">
            {campaigns.map((campaign) => (
              <div
                key={campaign.id}
                className="bg-white rounded-lg md:rounded-xl border border-gray-200 p-4 md:p-6 hover:shadow-lg transition-shadow flex flex-col"
              >
                {/* Header */}
                ...
                {/* Campaign Info */}
                <h3 className="text-base md:text-lg font-bold text-gray-900 mb-2 break-words">{campaign.name}</h3>
                <p className="text-xs md:text-sm text-gray-600 mb-3 md:mb-4 line-clamp-2 break-words">
                ...
                <div className="flex flex-col gap-2 mt-auto">
                  ...
                  <Button
                    ...
                    className="w-full bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 text-white text-sm md:text-sm flex items-center justify-center gap-2 py-2.5 md:py-2 font-semibold shadow-md h-11 md:h-auto"
                  >
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      className="flex-1 bg-blue-100 hover:bg-blue-200 text-blue-800 text-xs md:text-sm flex items-center justify-center gap-1 py-2.5 md:py-2 h-10 md:h-auto"
                    >
                      <BarChart3 className="w-3.5 h-3.5 md:w-3 md:h-3 flex-shrink-0" />
                      <span className="truncate">Analytics</span>
                    </Button>
                    <Button
                      onClick={() => handleEditCampaign(campaign)}
                      className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-900 text-xs md:text-sm flex items-center justify-center gap-1 py-2.5 md:py-2 h-10 md:h-auto"
                    >
                      <Edit className="w-3.5 h-3.5 md:w-3 md:h-3 flex-shrink-0" />
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      className={`flex-1 text-xs md:text-sm flex items-center justify-center gap-1 py-2.5 md:py-2 h-10 md:h-auto ${
                    ...
                    >
                      {campaign.status === 'active' ? (
                        <>
                          <Pause className="w-3.5 h-3.5 md:w-3 md:h-3 flex-shrink-0" />
                          <span className="truncate">Pause</span>
                        </>
                      ) : (
                        <>
                          <Play className="w-3.5 h-3.5 md:w-3 md:h-3 flex-shrink-0" />
                    ...
                    <Button
                      ...
                      className="flex-1 bg-red-100 hover:bg-red-200 text-red-800 text-xs md:text-sm flex items-center justify-center gap-1 py-2.5 md:py-2 h-10 md:h-auto"
                    >
                      <Trash2 className="w-3.5 h-3.5 md:w-3 md:h-3 flex-shrink-0" />
